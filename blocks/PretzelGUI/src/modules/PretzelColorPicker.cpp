...
void PretzelColorPicker::setup() {
  ...
  mHueSurf = ci::Surface(loadImage(loadAsset(PRETZEL_HSL_BAR)));
  ...
  int swatchSize = 150;
  mColorSwatchRect =
      Rectf(mColorPickRect.x2 - swatchSize, mColorPickRect.y2, mColorPickRect.x2, mColorPickRect.y2 + swatchSize);
...
void PretzelColorPicker::mouseDown(const ci::vec2 &pos) {
  if (!bExpanded && mCollapsedRect.contains(pos - mOffset)) {
    ...
    mCrosshairPos.x =
        ci::math<float>::clamp((localPos.x - mColorSwatchRect.x1) / mColorSwatchRect.getWidth(), 0.0, 1.0);
    mCrosshairPos.y =
        ci::math<float>::clamp((localPos.y - mColorSwatchRect.y1) / mColorSwatchRect.getHeight(), 0.0, 1.0);
    bDragging = true;
...
void PretzelColorPicker::updateBounds(const ci::vec2 &offset, const ci::Rectf &parentBounds) {
  ...
  // large picker box
  int swatchSize = 150;
  mColorSwatchRect = Rectf(mHueStripRect.x2 + 10,
                           mColorPickRect.y2,
                           mHueStripRect.x2 + 10 + swatchSize,
                           mColorPickRect.y2 + swatchSize);
  mColorSwatchRect.offset(vec2(-1, 1));
