...
// =========================================================================================================================================
template<typename T>
void PSliderT<T>::setup(const std::string label,
                        T *value,
                        const T min,
                        const T max,
                        ci::vec2 sliderLeft,
...
// =========================================================================================================================================
PretzelSlider::PretzelSlider(BasePretzel *parent, std::string labelText, int *value, int minVal, int maxVal)
    : BasePretzel() {
...
PretzelSlider::PretzelSlider(BasePretzel *parent, std::string labelText, float *value, float minVal, float maxVal)
    : BasePretzel() {
  mBounds.set(0, 0, 200, 30);
...
PretzelSlider::PretzelSlider(BasePretzel *parent, std::string labelText, vec2 *value, vec2 minVal, vec2 maxVal)
    : BasePretzel() {
...
PretzelSlider::PretzelSlider(BasePretzel *parent, std::string labelText, vec3 *value, vec3 minVal, vec3 maxVal)
    : BasePretzel() {
  mLineOffset = vec2(0, 14);
  mBounds.set(0, 0, 200, 30);
  ...
  gl::pushMatrices();
  {
    ...
    if (mSliderListf.size() > 1) {
      ...
      for (auto it = mSliderListf.begin(); it != mSliderListf.end(); ++it) {
        ...
        mGlobal->renderTextRight(mGlobal->to_string_with_precision(it->getValue()),
                                 mBounds.getUpperRight() + vec2(-12, 1));
        it->draw();
      }
    }
